...
export const authMiddleware: FastifyPluginAsync = async (fastify) => {
  fastify.addHook('preHandler', async (request, reply) => {
    ...
    if (!authHeader) {
      return reply.status(401).send({ success: false, message: '未提供token' });
    }

    const token = authHeader.replace('Bearer ', '');
    if (!token) {
      return reply.status(401).send({ success: false, message: 'token格式错误' });
    }
    ...
    try {
      ...
      if (!user) {
        return reply.status(401).send({ success: false, message: '用户不存在' });
      }
