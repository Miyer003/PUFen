...
export class RewardStageService {
  ...
  static async getUserAvailableStages(): Promise<{
    ...
    try {
      const stage2Status = await this.checkAndUnlockStage2();
  ...
  static async getStageStats(): Promise<{
    ...
    try {
      ...
      const [stage1Total, stage1Available, stage1SoldOut] = await Promise.all([
        rewardItemRepo.count({ where: { stage: 1 } }),
        rewardItemRepo.count({ where: { stage: 1, stock: MoreThan(0) } }),
        rewardItemRepo.count({ where: { stage: 1, stock: 0 } })
      ]);
      
      // 第二阶段统计
      const [stage2Total, stage2Available, stage2SoldOut] = await Promise.all([
        rewardItemRepo.count({ where: { stage: 2 } }),
        rewardItemRepo.count({ where: { stage: 2, stock: MoreThan(0) } }),
        rewardItemRepo.count({ where: { stage: 2, stock: 0 } })
      ]);
